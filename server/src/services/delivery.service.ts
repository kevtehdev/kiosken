...
export class DeliveryService {
    ...
    async processNewOrder(details: DeliveryDetails): Promise<void> {
        ...
        try {
            ...
            console.log('Orderbekräftelse skickad till kund:', details.customerEmail);
    ...
    private formatOrderDetails(details: DeliveryDetails): string {
        const formattedItems = details.items.map(item => 
            `${item.quantity}x ${item.name} (${item.price.toFixed(2)} kr/st) - Totalt: ${item.totalPrice.toFixed(2)} kr`
        ).join('\n');
