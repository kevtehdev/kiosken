...
export class OAuthController {
    ...
    public authorize = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            logger.info(
                "Generated authorization URL and saved verifier to session"
            );
    ...
    public callback = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!code || !oauthSession?.codeVerifier) {
                throw new Error(
                    "Ogiltig OAuth callback - saknar code eller code_verifier"
                );
            }
            ...
            redirectUrl.searchParams.set(
                ...
                error instanceof Error
                    ? error.message
                    : "Kunde inte slutföra Onslip-integrationen"
            );
